...
const useFlight = (props) => {
  ...
  const handleRowClick = (params, event) => {
    ...
    const isActionIcon =
      target.classList.contains("action-icon") ||
  ...
  const deleteFlight = async (id) => {
    await FlightsService.deleteFlight(id).then((response) => {
      setFlights(flights.filter((el) => el.id !== id));
    });
  };

  const getFlights = async () => {
    const response = await FlightsService.getFlights().catch((err) =>
